...
<CardGrid>
	...
	<KitCard title="fetch-kit" icon="lucide:cloud-download" href="/fetch-kit/overview/">
		A typed `fetch` wrapper plus two React hooks. Retries with backoff, timeouts via `AbortController`, an error class hierarchy you can `instanceof`, optional schema validation, request/response interceptors. The React layer is a separate subpath — non-React consumers don't pay for it.
	</KitCard>
